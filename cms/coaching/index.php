<?php
  ...
  $results = $db->query("SELECT * FROM users WHERE supervisor_name = '$username' AND position = 'Agent' ORDER BY user_fullname ASC");

  $results1 = $db->query("SELECT * FROM users WHERE username = '$username'");
  while ($row1 = $results1->fetch_assoc()) {
    $om_name = $row1['supervisor_name'];
  }

  $results1 = $db->query("SELECT * FROM users WHERE username = '$om_name'");
  while ($row1 = $results1->fetch_assoc()) {
    $om_name1 = $row1['user_fullname'];
  }
...
<script src="../../scripts/script_upload_image.js"></script>
		<script>
...
<form id="uploadimage" action="" method="post" name="form" enctype="multipart/form-data">
  <input type="hidden" name="hidden1" value = "<?php echo $om_name; ?>"/>
      			<div id="body3">
              <span class="smart">Agent's Name:
